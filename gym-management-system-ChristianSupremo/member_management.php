...
$sql = "SELECT Member.MemberID, Member.Name, Plan.PlanID, Plan.PlanName, Plan.Rate, Membership.StartDate, Membership.EndDate, Membership.Status
        FROM Membership
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                ...
                while ($plan = $plans->fetch_assoc()) {
                    // Check if the current plan is selected for this member
                    $selected = ($plan['PlanID'] == $row['PlanID']) ? 'selected' : '';
